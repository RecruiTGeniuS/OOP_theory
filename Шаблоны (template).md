...
`typename` - ключевое слово, которое обозначает, что шаблон является "заполнителем" для типа данных. Можно заменить на `class` и ничего не изменится, но с точки зрения семантики шаблоны для классов лучше писать с ключевым словом `class`, а для всего остального с `typename`.  
...
```
 ...
 и он сделает всё не явно, но некоторые компиляторы плохие и могут выдать ошибку на такое, поэтому лучше это прописывать.