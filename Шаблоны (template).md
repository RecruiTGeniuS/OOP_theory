...
```
Как видно, тут в параметрах у нас тип данных `int` и получается, в том случае, если нам надо будет работать, к примеру, с `double` то придётся или переписывать ту же функцию, только изменёнными параметрами, то есть перегружать её, или писать новую функцию, с новыми параметрами.
...
>Шаблон - это формочка для печенья, которая указывает, как вырезать печенье, которое выглядит почти одинаково (хотя печенье может быть сделано из разных видов теста, все они будут иметь одинаковую основную форму). Ну или Шаблон - это термос, а дальше и так понятно. 
